...
本ツールは **ブラウザ上のWebアプリ** または **コマンドライン（CLI）** の両方で利用できます。

### 1. Webアプリとして使う（推奨）

ブラウザ上でリアルタイムにプレビューしながら編集し、ワンクリックでPNGや一括ZIPをダウンロードできます。

```bash
npm run dev
```

ブラウザで `http://localhost:5173` を開くだけで利用可能です。
- **リアルタイム縮小プレビュー**: 入力した内容やBudouXの日本語折り返しがその場で反映されます。
- **セッション編集・並び替え**: 登壇者やタイトルの編集、↑/↓ボタンでの発表順変更、追加・削除が直感的に行えます。
- **JSONの読込・保存**: 手元の `schedule.json` をドラッグ＆ドロップで読み込み、保存も可能です。
- **画像ダウンロード**: 「表示中スライドの保存 (PNG)」や「全スライドの一括ダウンロード (ZIP)」が可能です。
- **静的ビルド**: `npm run build` を実行すると `dist/` に静的ファイルが出力され、GitHub Pages 等にそのまま公開できます。
